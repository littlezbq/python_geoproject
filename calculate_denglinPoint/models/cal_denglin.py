...
from calculate_denglinPoint.utils.cdld import calculateDenglin
import numpy as np
...
def interface_calDenglin(dem_path, denglin_num, time_limit, divideSpace, demAxis):
    """
    计算可达域内点的可视域并渲染至可达域上
    :param dem:
    :param remoteDomin:
    ...
    # 设置登临点算法计算保存的程序基路径
    ascensionpoint_basepath = os.path.join(DenglinConsts.ASCENSIONLIST_SAVEPATH,
    ...
    # 计算登临点,返回登临点列表
    cal_dl = calculateDenglin(x_y, dem_path,time_limit)
    ascensionpoint_resultList = cal_dl.calculateDenlin(divideSpace, demAxis)
    ...
    # 前denglin_num个登临点列表
    ascensionpoint_numlist_relative = [(float(ascensionpoint_resultList[i][0]),float(ascensionpoint_resultList[i][1])) for i in range(denglin_num)]


    # 保存前ascensionpoint_num个登临点的可视域列表
    ascensionpoint_viewshedpathlist = ["".join((ascensionpoint_basepath,'/', str(int(point[0])), '_',str(int(point[1])), 'ascensionpoint_viewshed.png')) for point in ascensionpoint_numlist_relative]
    ...
    for i in range(len(ascensionpoint_numlist_relative)):
        transpoint, _ = tl.coordinateTransform(dem_path, ascensionpoint_numlist_relative[i][0], ascensionpoint_numlist_relative[i][1])
...
def interface_calBaolu(dem_path,cur_dis,exposiveNum):
    """"

    """
    # baolu_point_areaPath = os.path.join(DenglinConsts.ASCENSIONLIST_SAVEPATH, 
    #                                 os.path.join(os.path.basename(dem_path).split('.')[0],str(cur_dis)+'_'+
    #                                              str(exposiveNum)+ 'baoluPointArea.png'))

    # 读取可达域内点的列表
    domainPoint_path = os.path.join(DenglinConsts.VIEWSHED_PATH, "".join((os.path.basename(dem_path).split('.')[0],"/",str(cur_dis))))
    
    x_y = np.loadtxt(domainPoint_path + "/accessdomainpoint_index.txt", dtype=int)

    # 计算出可达域内可视域叠加情况，返回可视域叠加图
    cal_dl = calculateDenglin(x_y, dem_path, cur_dis)
    result_baolu = cal_dl.calculateBaolu(cur_dis)
    ...
    # 获取哪些点能看到第一个暴露点
    parent_list = []
    viewshedPath = r"D:\projects\webGisProject-dev20220412\calculate_denglinPoint\middle\viewshed\SX3_005_QMC\15"
    exposivepoint = [int(exposivepoints_numlist_relative[0][0]), int(exposivepoints_numlist_relative[0][1])]
    for file in os.listdir(viewshedPath):
        if file.endswith("tif"):
            viewsheddata = np.array(Image.open(os.path.join(viewshedPath, file)))
    ...
    np.savetxt(r"D:\projects\webGisProject-dev20220412\calculate_denglinPoint\middle\viewshed\SX3_005_QMC\15\exposiveParent.txt",np.array(parent_list))
...
if __name__ == "__main__":
    remotename = r"D:\projects\webGisProject-dev20220412\static\datasets\SX3_005_QMC.png"
    demname = r"D:\projects\webGisProject-dev20220412\static\datasets\SX3_005_QMC.tif"

    print(interface_calBaolu(demname, 15, 1))