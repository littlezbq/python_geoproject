from road_intersection.road_extraction.nets.densenet_sp import DenseNet, _Transition, _load_state_dict
import torch
from torch import nn
from collections import OrderedDict
from road_intersection.road_extraction.nets.strip_pooling import StripPooling
...
class _DenseUNetEncoder(DenseNet):
    ...
    def forward_hook_fn(self, module, input_, output):
        self.skip_connections.append(input_[0])
...
class _DenseUNetDecoder(DenseNet):
    def __init__(self, skip_connections, growth_rate, block_config, num_init_features, bn_size, drop_rate, upsample):
        ...
        #         引入条纹池化模块
        # self.features.add_module("strippool", StripPooling(num_init_features, up_kwargs={'mode': 'bilinear', 'align_corners': True}))
...
class _TransitionUp(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, skip_connections):
        ...
        #         引入条纹池化模块
        self.add_module("strippool",
                        StripPooling(num_output_features, up_kwargs={'mode': 'bilinear', 'align_corners': True}))
...
class DenseUNet(nn.Module):
    ...
    def freeze_backbone(self):
        #     冻结Encoder中除了条纹池化层的其他层
        for block in self.encoder.features.modules():
            if "StripPooling" != block._get_name():
...
if __name__ == "__main__":
    ...
    model = DenseUNet(2, drop_rate=0.3)
    model = model.cuda()
    ...
    # writer.add_graph(model, input_to_model=torch.rand(1, 3, 256, 256).cuda())
    # writer.close()
